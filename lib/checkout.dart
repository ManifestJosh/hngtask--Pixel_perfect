...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  void payment() {
    ...
    double totalPrice = widget.checkoutList.fold(0, (sum, item) {
      return sum + item['quantity'] * item['price'];
    });
    double total = totalPrice + delivery;

    List<Map<String, dynamic>> formattedItems = widget.checkoutList.map((item) {
      return {
    ...
    Order newOrder = Order(
      items: formattedItems, // Save as JSON string
      totalPrice: total,
      date: DateTime.now(),
    );
    ...
    Get.to(() => PaymentSuccessPage(), arguments: newOrder);
  }
